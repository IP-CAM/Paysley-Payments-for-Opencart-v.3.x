<?php
namespace Opencart\Admin\Controller\Extension\Paysley\Payment;

/**
 * Class Paysley
 *
 * @package Opencart\Admin\Controller\Extension\Paysley\Payment
 */
...
class Paysley extends \Opencart\System\Engine\Controller {
	 ...
	 * @var string $code
	 */
	private $code = 'paysley';

		
	/**
	 * this function is the constructor of ControllerExtensionPaymentPaysley class
	 *
	 * @return  void
	 */
	public function index() {
		$this->load->language('extension/paysley/payment/' . $this->code);
		...
		$this->load->model('extension/paysley/payment/paysley');

		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {

			$lang_id = $this->model_extension_paysley_payment_paysley->getLangId();
		...
		$data['custom_fields'] = $this->model_extension_paysley_payment_paysley->getCustomFields();
		$data['custom_field_values'] = $this->model_extension_paysley_payment_paysley->getCustomFieldValues();
		...
		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('extension/paysley/payment/' . $this->code, 'user_token=' . $this->session->data['user_token'], true),
			'separator' => ' :: '
		);

		$data['action'] = $this->url->link('extension/paysley/payment/' . $this->code, 'user_token=' . $this->session->data['user_token'], true);
		...
		$this->response->setOutput($this->load->view('extension/paysley/payment/' . $this->code, $data));
	}
	...
	public function install() {
		...
		$this->load->model('setting/setting');
		$this->load->model('extension/paysley/payment/paysley');
		$this->model_extension_paysley_payment_paysley->addCustomOrderStatuses();
		$this->model_extension_paysley_payment_paysley->install();
	}
	...
	public function validate() {
		if (!$this->user->hasPermission('modify', 'extension/paysley/payment/paysley')) {
			$this->error['warning'] = $this->language->get('ERROR_PERMISSION');
			return false;
		}
		if (empty($this->request->post['payment_paysley_access_key'])) {
